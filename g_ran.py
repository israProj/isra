...
graph_num = 0
gen_model_saved = "gen_model-g_ran-tmp"
...
def work():
    ...
    def rand_shape():
        ...
        fixed_shape = [ran(1, MAX_TENSOR_DIM_LEN + 1)
    ...
    def pass_value(t, given_value=True):
        ...
        if NO_INPUT or given_value:
            init_tensor.append(helper.make_tensor(
                t.name, t_type, dims=t_value.shape, vals=t_value.flatten()))
        else:
    ...
    for step in range(network_node_num):
        ...
        if new_node_type in ['MaxPool', 'AveragePool', 'LpPool']:
            kwargs['kernel_shape'] = [ran(1, MAX_TENSOR_DIM_LEN + 1)
                                      for i in range(n1_dim - 2)]
            kwargs['strides'] = [ran(1, MAX_TENSOR_DIM_LEN + 1)
                                 for i in range(n1_dim - 2)]
        ...
        if new_node_type == 'Split':
            ...
            n2_shape = [n2_dim]
            n2 = new_tensor(n2_shape, TensorProto.INT64, np.array(
                [ran(1, MAX_TENSOR_DIM_LEN) for i in range(n2_dim)]))
            pass_value(n2)
            ...
            for i in range(nums_output):
                out_n_shape = [ran(1, MAX_TENSOR_DIM_LEN + 1)
            ...
            indices = new_tensor(indices_shape, TensorProto.INT64,
                                 np.array(indices_value))
            pass_value(indices)
            ...
            ends_shape = [n1_dim]
            ends_value = [starts_value[i] +
                          ran(0, n1_shape[i] - starts_value[i]) + 1 for i in range(n1_dim)]
            axes_shape = [n1_dim]
            ...
            starts = new_tensor(
                starts_shape, TensorProto.INT64, np.array(starts_value))
            pass_value(starts)
            ends = new_tensor(ends_shape, TensorProto.INT64,
                              np.array(ends_value))
            pass_value(ends)
            axes = new_tensor(axes_shape, TensorProto.INT64,
                              np.array(axes_value))
            pass_value(axes)
            steps = new_tensor(steps_shape, TensorProto.INT64,
                               np.array(steps_value))
            pass_value(steps)
            ...
            scales_dim = 1
            scales_shape = [n1_dim]
            scales = new_tensor(scales_shape, TensorProto.FLOAT,
                                np.array([1.0 for i in range(n1_dim)]))
            pass_value(scales)
            ...
            for i in range(n1_dim):
                roi_value.append(1)
            roi = new_tensor(roi_shape, TensorProto.FLOAT,
                             np.array([roi_value]))
            pass_value(roi)
            ...
            n2_dim = 1
            n2_shape = [new_dim]
            n2 = new_tensor(n2_shape, TensorProto.INT64,
                            np.array(reshape_shape))
            pass_value(n2)
            ...
            for i in range(n1_dim):
                pad_n = ran(0, 3)
                pads_value += [pad_n]
            pads = new_tensor([2 * n1_dim], TensorProto.INT64,
                              np.array(pads_value))
            pass_value(pads)
            ...
            n2 = new_tensor(n2_shape, TensorProto.BOOL,
                            np.array(compress_shape))
            pass_value(n2)
            inputs = [n1, n2]
        ...
        new_node = helper.make_node(
            new_node_type, inputs=inputs, outputs=outputs, name=node_name, **kwargs)
    ...
    for x in no_succ:
        ...
        output_ts.append(x2)
        n = helper.make_node('Flatten', inputs=[x], outputs=[
                             x2], name='flatten_%s' % x, axis=0)
        node_list.append(n)
        tot_output_element += totElement(x)

    final_tensor = new_tensor([1, tot_output_element])
    n = helper.make_node('Concat', inputs=output_ts, outputs=[
                         final_tensor], name='concat_outputs', axis=-1)
    node_list.append(n)
    ...
    graph_def = helper.make_graph(
        node_list, "test-model", input_tensor, output_tensor, init_tensor)
...
if __name__ == '__main__':
    ...
    f_timename = './models/baseline/rand/'+str(MIN_NODE)+'_'+str(MAX_NODE)+'_'+str(pickExistRate)+ \
        '/time_rand_'+str(MIN_NODE)+'_'+str(MAX_NODE)+'_'+str(pickExistRate)+'_e'+str(args.loop)+'.txt'

    model_prefix = './models/baseline/rand/'+str(MIN_NODE)+'_'+str(MAX_NODE)+'_'+str(pickExistRate)+ \
        '/'+str(args.loop)+'/rand_'+str(MIN_NODE)+'_'+str(MAX_NODE)+'_'+str(pickExistRate)+ \
        '_e'+ str(args.loop)+'_'
    if os.path.exists(f_timename):
        os.remove(f_timename)
    ...
    create_folder(f_timename)

    for iter in range(ITER_NUM):
        while True:
            start_time = time.perf_counter()
            model, p1_, p2_ = work()
            ok = valid([model, p1_, p2_])
            end_time = time.perf_counter()
            total_time += (end_time-start_time)
            gen_models += 1
            if ok:
                f_time = open(f_timename, 'a+')
                f_time.write('at time ' + str(total_time) + ' generate ' + str(iter+1) +
                        ' valid models out of ' + str(gen_models) + ' models\n')       
                f_time.close()
